...
def start():
    ...
    try:
        ...
        raise PermissionError(
            f"Permission denied: Unable to copy to {destination_path}"
        )
...
def end():
    ...
    iterations_path = (
        "/workspace/tests/integration_tests/fx_classify/pipeline_0/iterations"
    )
    ...
    prepped_data_path = (
        "/workspace/tests/integration_tests/fx_classify/pipeline_0/prepped_data"
    )
    ...
    weights_dict_path = (
        "/workspace/tests/integration_tests/fx_classify/pipeline_0/weights_dict"
    )
...
class TestFxClassify0(unittest.TestCase):
    ...
    def test_inspect(self, mock_show):
        ...
        inspect_cases = [
            {
                "data_keys": ["hourly_features"],
                "plot_features": True,
                "plot_mode": "rate",
            },
            {
                "data_keys": ["tech_features"],
    ...
    def test_align(self):
        ...
        conn = sqlite3.connect(
            "/workspace/tests/integration_tests/fx_classify/fixtures/fx_classify_test_train_data_clean.db"
        )
        cursor = conn.cursor()
    ...
    def test_engineer(self):
        ...
        targets = pd.read_sql(
            "SELECT target FROM targets ORDER BY date, pair LIMIT 5", conn
        )
        targets = targets.astype({"target": "string"})
        expected_targets = pd.DataFrame({"target": [0, 0, 0, 0, 2]}).astype(
            {"target": "string"}
        )
        pd.testing.assert_frame_equal(targets, expected_targets)

        engineered = pd.read_sql(
            "SELECT * FROM test_engineered_feature_data ORDER BY date, pair LIMIT 5",
            conn,
        )
    ...
    def test_predict_and_store(self):
        ...
        conn = sqlite3.connect(
            "/workspace/tests/integration_tests/fx_classify/pipeline_0/predictions/14_03_2025_23_02_54_epoch4.db"
        )
        pred = pd.read_sql(
            """SELECT confidence FROM predictions ORDER BY date, pair LIMIT 5""", conn
    ...
    def test_report_candidates(self, mock_show):
        ...
        eval = Eval()
        eval.report_candidates(mode="convert", accuracy_threshold=0.35, volume=10)

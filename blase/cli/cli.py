...
def create_project(template_type, project_name):
    if template_type not in TEMPLATE_CHOICES:
        print(
            f"Error: Template '{template_type}' not found. Available options: {TEMPLATE_CHOICES}"
        )
        return
    ...
    print(
        f"Project '{project_name}' created using template '{template_type}' at: {dest_path.resolve()}"
    )
...
def main():
    ...
    * ``--step`` vs ``--data``: mutually exclusive targets
    ...
    create_parser.add_argument(
        "template", choices=TEMPLATE_CHOICES, help="Type of project template"
    )
    ...
    rst = subparsers.add_parser(
        "restore", help="Restore/inspect recorded steps and data"
    )
    ...
    rn.add_argument("--run")
    rn.add_argument(
        "--mode", choices=["verify", "materialize", "replay"], default="verify"
    )

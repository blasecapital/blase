...
def install_fake_pillow(monkeypatch, mapping):
    ...
    monkeypatch.setattr(
        importlib.util,
        "find_spec",
        lambda name, package=None: object() if name == "PIL" else None,
        raising=True,
    )
    return PIL
...
def test_tensor_prefers_torch_cuda(monkeypatch):
    ...
    class DummyCUDA:
        ...
        @staticmethod
        def mem_get_info():
            return (1_000_000_000, 2_000_000_000)  # free, total
...
def test_tensor_tf_uses_pynvml(monkeypatch):
    ...
    sys.modules["pynvml"] = make_module(
        "pynvml", **{k: getattr(NVML, k) for k in dir(NVML) if not k.startswith("_")}
    )
...
def test_tensor_tf_virtual_device_when_no_pynvml(monkeypatch):
    ...
    class TFExperimental:
        @staticmethod
        def get_virtual_device_configuration(_gpu):
            return [VCfg]

        @staticmethod
        def get_memory_info(_):
            return {"current": 100 * 2**20}  # 100 MiB
...
def test_happy_path_headers_sorted_and_fields(tmp_path, monkeypatch):
    ...
    a = tmp_path / "a.jpg"
    a.write_bytes(b"X" * 123)
    Path(a).touch()
    b = tmp_path / "b.png"
    b.write_bytes(b"Y" * 45)
    Path(b).touch()
    c = tmp_path / "c.gif"
    c.write_bytes(b"Z" * 1)
    Path(c).touch()
    ...
    # Act
    items = scan_manifest_headers(
        str(tmp_path), "**/*", recursive=False, filename_filter=None
    )
    ...
    assert (
        isinstance(a_rec["hash_path_mtime"], str)
        and len(a_rec["hash_path_mtime"]) == 64
    )


def test_filename_filter_excludes(tmp_path, monkeypatch):
    f1 = tmp_path / "keep.jpg"
    f1.write_bytes(b"1")
    f2 = tmp_path / "skip.jpg"
    f2.write_bytes(b"2")
    install_fake_pillow(
        monkeypatch,
        {
            "keep.jpg": {"size": (1, 1), "mode": "RGB"},
    ...
    items = scan_manifest_headers(
        ...
        "*.jpg",
        recursive=False,
        filename_filter=lambda p: not p.endswith("skip.jpg"),
    )
...
def test_recursive_double_star_stripped_when_recursive_false(tmp_path, monkeypatch):
    ...
    install_fake_pillow(
        monkeypatch,
        {
            "top.jpg": {"size": (1, 1), "mode": "RGB"},
            "deep.jpg": {"size": (1, 1), "mode": "RGB"},
        },
    )
    patch_hash_in_module(monkeypatch)

    items_no_rec = scan_manifest_headers(
        str(tmp_path), "**/*.jpg", recursive=False, filename_filter=None
    )
    assert [it["rel_path"] for it in items_no_rec] == ["top.jpg"]

    items_rec = scan_manifest_headers(
        str(tmp_path), "**/*.jpg", recursive=True, filename_filter=None
    )
...
def test_dependency_missing_raises(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        importlib.util, "find_spec", lambda name, package=None: None, raising=True
    )
...
def test_bad_directory_raises(monkeypatch, tmp_path):
    ...
    with pytest.raises(NotADirectoryError):
        scan_manifest_headers(
            str(tmp_path / "nope"), "*.jpg", recursive=False, filename_filter=None
        )
...
def test_deterministic_ordering(tmp_path, monkeypatch):
    ...
    install_fake_pillow(
        monkeypatch, {n: {"size": (1, 1), "mode": "RGB"} for n in names}
    )
    patch_hash_in_module(monkeypatch)
...
def test_pms_mode_requires_field(tmp_path):
    ...
    with pytest.raises(ValueError):
        compute_manifest_root_hash(
            bad, str(tmp_path), "**/*.jpg", True, 0, "path_mtime_size"
        )
...
def test_scan_params_affect_hash_each(tmp_path):
    ...
    other_dir.mkdir()
    assert (
        compute_manifest_root_hash(m, str(other_dir), "**/*.jpg", True, 42, "content")
        != base
    )

    # different pattern
    assert (
        compute_manifest_root_hash(m, str(tmp_path), "*.jpg", True, 42, "content")
        != base
    )

    # different recursive flag
    assert (
        compute_manifest_root_hash(m, str(tmp_path), "**/*.jpg", False, 42, "content")
        != base
    )

    # different seed
    assert (
        compute_manifest_root_hash(m, str(tmp_path), "**/*.jpg", True, 7, "content")
        != base
    )
    ...
    assert (
        compute_manifest_root_hash(
            m_pms, str(tmp_path), "**/*.jpg", True, 42, "path_mtime_size"
        )
        != base
    )
...
def test_seed_none_vs_same_seed(tmp_path):
    m = _content_items()
    h_none = compute_manifest_root_hash(
        m, str(tmp_path), "**/*.jpg", True, None, "content"
    )
    h_42a = compute_manifest_root_hash(
        m, str(tmp_path), "**/*.jpg", True, 42, "content"
    )
    h_42b = compute_manifest_root_hash(
        m, str(tmp_path), "**/*.jpg", True, 42, "content"
    )
    assert h_42a == h_42b
...
def test_empty_manifest_hashes_on_scan_identity_only(tmp_path):
    h1 = compute_manifest_root_hash(
        [], str(tmp_path), "**/*.jpg", True, None, "content"
    )
    h2 = compute_manifest_root_hash(
        [], str(tmp_path), "**/*.jpg", True, None, "content"
    )
    assert h1 == h2
...
def test_max_side_downscale(tmp_path):
    ...
    arr = decode_batch_pil(
        [{"abs_path": str(p)}], return_type="np", color="rgb", max_side=200
    )[0]
...
def test_tensor_happy_path(tmp_path):
    ...
    assert (
        isinstance(t, _torch.Tensor)
        and t.dtype == _torch.uint8
        and t.shape[-1] in (1, 3)
    )
...
def test_rgb_basic_shape_and_color(tmp_path):
    ...
    out = decode_batch_cv2(
        [{"abs_path": str(p)}], return_type="np", color="rgb", max_side=None
    )
    arr = out[0]

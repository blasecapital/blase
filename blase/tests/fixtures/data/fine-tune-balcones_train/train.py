...
class Train:
    ...
    def _initial_bias(self):
        ...
        if not os.path.exists(self.weight_dict_path):
            raise FileNotFoundError(
                f"Weight dictionary not found at: {self.weight_dict_path}"
            )
    ...
    def load_model(self):
        ...
        create_model = self._import_function(
            self.model_modules_path, self.model_function
        )
    ...
    def compile_model(self, model):
        ...
        if isinstance(optimizer_config, dict):
            optimizer_type = optimizer_config.pop(
                "type", "adam"
            )  # Default to Adam if type is not specified
            optimizer_class = getattr(
                tf.keras.optimizers, optimizer_type.capitalize(), None
            )

            if optimizer_class is None:
                raise ValueError(
                    f"Optimizer type '{optimizer_type}' is not recognized."
                )
        ...
        if (
            self.custom_loss
            and isinstance(self.custom_loss, dict)
            and len(self.custom_loss) > 0
        ):
        ...
        # Compile the model
        model.compile(optimizer=optimizer, loss=loss_fn, metrics=self.metrics)
    ...
    def _load_tfrecord_dataset(self, feature_files, target_files):
        ...
        """
        feature_metadata = {
            category: self._load_feature_metadata(category)
        ...
        feature_datasets = {
            category: tf.data.TFRecordDataset(files).map(
                lambda x: self._parse_tfrecord_fn(
                    x, feature_metadata[category], category
                ),
        ...
        feature_tuple_dataset = feature_tuple_dataset.batch(self.batch_size).prefetch(
            tf.data.AUTOTUNE
        )
        full_target_dataset = full_target_dataset.batch(self.batch_size).prefetch(
            tf.data.AUTOTUNE
        )
    ...
    def _group_files_by_number(self, feature_category, dataset_type):
        ...
        tfrecord_files = sorted(
            [f for f in os.listdir(self.data_dir) if f.endswith(".tfrecord")]
        )
        ...
        target_files = [
            f for f in tfrecord_files if "targets" in f and dataset_type in f
        ]
    ...
    def _load_class_weights(self):
        ...
        if self.use_weight_dict and os.path.exists(self.weight_dict_path):
            with open(self.weight_dict_path, "r") as f:
                class_weights = json.load(f)
            class_weights = {
                int(k): v for k, v in class_weights.items()
            ...
            class_weights_tensor = tf.constant(
                [class_weights[i] for i in sorted(class_weights)], dtype=tf.float32
            )
    ...
    def _add_sample_weights(self, features, target, class_weights_tensor):
        ...
        sample_weight = tf.gather(
            class_weights_tensor, target
        )  # Lookup weight using target class
        return features, target, sample_weight

    def _val_prediction_data(
        self, model, val_dataset, full_target_dataset, target_column_names
    ):
        """
    ...
    def fit_model(self, model):
        ...
        if self.callback_function:
            aggregated_callbacks = self._import_function(
                self.model_modules_path, self.callback_function
            )
            if callable(aggregated_callbacks):
                # Create an instance of AggregatedCallbacks with any parameters you want.
                log_dir = os.path.join(
                    save_dir, "logs", datetime.now().strftime("%Y%m%d-%H%M%S")
                )
                aggregated_callbacks = aggregated_callbacks(
        ...
        for epoch in range(self.epochs):
            ...
            train_groups = {
                category: self._group_files_by_number(category, "train")
                for category in self.feature_categories
            }
            ...
            for num in sorted(train_groups[next(iter(self.feature_categories))].keys()):
                ...
                target_files = (
                    train_groups[next(iter(self.feature_categories))]
                    .get(num, {})
                    .get("targets", [])
                )
                ...
                if (
                    any(not files for files in feature_files.values())
                    or not target_files
                ):
                ...
                history = model.fit(
                    train_dataset, epochs=1, verbose=1, callbacks=callbacks
                )
            ...
            avg_train_loss = (
                train_loss_sum / num_train_batches if num_train_batches else 0
            )
            avg_train_acc = (
                train_acc_sum / num_train_batches if num_train_batches else 0
            )
            ...
            val_groups = {
                category: self._group_files_by_number(category, "val")
                for category in self.feature_categories
            }

            for num in sorted(val_groups[next(iter(self.feature_categories))].keys()):
                feature_files = {
                    category: val_groups[category].get(num, {}).get("features", [])
                    for category in self.feature_categories
                }
                target_files = (
                    val_groups[next(iter(self.feature_categories))]
                    .get(num, {})
                    .get("targets", [])
                )
                ...
                val_dataset, full_target_dataset, target_column_names = (
                    self._load_tfrecord_dataset(feature_files, target_files)
                )

                results = model.evaluate(
                    val_dataset, verbose=1, callbacks=val_callbacks
                )
                ...
                for i, (pred, actual, conf) in enumerate(
                    zip(predicted_categories, actual_targets, confidences)
                ):
                    pred_rows.append(
                        [epoch + 1, pred, actual, conf] + extra_target_values[i]
                    )
            ...
            if aggregated_callbacks and aggregated_callbacks.use_conf_matrix:
                ...
                aggregated_callbacks.conf_matrix_callback.log_conf_matrix(
                    epoch, predicted_categories, actual_targets
                )
            ...
            print(
                f"Appended predictions for epoch {epoch + 1} to CSV file: {predictions_csv_path}"
            )
            ...
            if aggregated_callbacks:
                ...
                aggregated_callbacks.on_aggregated_epoch_end(
                    epoch, aggregated_logs, model
                )

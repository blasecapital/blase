...
def run_save_to_csv_replay(
    *,
    ...
    """
    Replays a Load.save_to_csv step by *consuming* an upstream generator of
    (batch, last) and writing to the target path using a live Load() instance.
    """
    if upstream_gen is None:
        raise RuntimeError("Load.save_to_csv replay requires an upstream generator of (batch, last).")

    # Pull recorded target + backend; allow CLI overrides
    ...
    if not target:
        raise ValueError("No target path available for Load.save_to_csv replay.")

    # Execute the write loop untracked
    ld = Load()
    # Write to a temp path, validate hash if we have one recorded, then move
    tmp = target.with_suffix(target.suffix + ".tmp")
    if tmp.exists():
        tmp.unlink()

    # consume to tmp (no tracking)
    last_seen = False
    ...
    # hash/verify if we have the recorded output
    # (optional but recommended)
    from blase.restoring import store
    outs = store.load_step_outputs(run_path, params.get("step_hash") or "")  # or pass outputs in
    if outs:
        recorded = outs[0]["data_hash"]
        from blase.utils.hashing import Hash
        if Hash().hash_file(tmp) != recorded:

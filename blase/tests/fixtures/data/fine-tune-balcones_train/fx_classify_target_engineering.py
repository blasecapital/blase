...
def normalize_pair_name(pair_name):
    ...
    base_name = re.sub(
        r"[^a-zA-Z]", "", pair_name
    ).upper()  # Remove non-alphabetic chars and convert to upper case
...
def add_open_prices(df, pair_name, multipliers, widemultipliers):
    ...
    def calculate_open_price_ask(row):
        ...
        factor, _ = get_multiplier(
            date, normalized_pair_name, multipliers, widemultipliers
        )
...
def calc_sl_tp(df, pair):
    ...
    df["avg_true_range"] = df["true_range"].shift(1).rolling(window=4).mean() * (
        1 + spread
    )
    ...
    df["buy_sl"] = df["open_price_ask"] - np.maximum(
        df["avg_true_range"], df["open_price_ask"] * 0.00125
    )
...
def true_outcome(index, df, multipliers, widemultipliers, n):
    ...
    for i in range(index, min(index + n, len(df) - 1)):
        ...
        effective_multiplier = (
            1 + multiplier if not is_wide_multiplier else 1 + is_wide_multiplier
        )
        ...
        if (
            next_row["high"] * effective_multiplier > sell_stop_loss
            and not sell_sl_time
        ):
        ...
        if (
            not sell_stop_loss_hit
            and next_row["low"] * effective_multiplier <= sell_take_profit
        ):
            outcome = "sell"
            hours_passed = (current_timestamp - date).total_seconds() / 3600
            break  # Take profit was hit, stop checking further
...
storage_map = {"targets": ["target", "hours_passed", "buy_sl_time", "sell_sl_time"]}

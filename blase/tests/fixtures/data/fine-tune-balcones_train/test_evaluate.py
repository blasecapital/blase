...
def test_function():
    ...
    def test_group_pred_files(self):
        ...
        with tempfile.TemporaryDirectory() as test_dir:
            ...
            for i in range(3):
                open(
                    os.path.join(self.eval.data_dir, f"file_{i}_train.tfrecord"), "w"
                ).close()
                open(
                    os.path.join(self.eval.data_dir, f"targets_{i}_train.tfrecord"), "w"
                ).close()

            feature_category = "file"
            result = self.eval._group_pred_files(
                self.eval.data_dir, feature_category, mode
            )

            expected_result = {
                0: {
                    "features": [f"{self.eval.data_dir}/file_0_train.tfrecord"],
                    "targets": [f"{self.eval.data_dir}/targets_0_train.tfrecord"],
                },
                1: {
                    "features": [f"{self.eval.data_dir}/file_1_train.tfrecord"],
                    "targets": [f"{self.eval.data_dir}/targets_1_train.tfrecord"],
                },
                2: {
                    "features": [f"{self.eval.data_dir}/file_2_train.tfrecord"],
    ...
    def test_parse_tfrecord_fn(self):
        ...
        category = "test"
        feature_categories = {"test": {"reshape": False, "shape": None}}
        result = self.eval._parse_tfrecord_fn(
            serialized_example, feature_metadata, category, feature_categories
        )
    ...
    def test_load_model(
        self, mock_initial_bias, mock_model_config, mock_import_function
    ):
        ...
        mock_import_function.assert_called_once_with(
            "/mock/dir/test_custom_train_funcs.py", "create_model"
        )
        mock_model_config.assert_called_once_with(config)
        mock_initial_bias.assert_called_once_with(
            mock_model_args, mock_weight_dict_path
        )
    ...
    def test_load_weights(self):
        ...
        config = {"dir": "/mock", "model_weights": "weights.h5"}
    ...
    def test_group_lime_files(self):
        ...
        with tempfile.TemporaryDirectory() as test_dir:
            ...
            expected_result = {
                0: {
                    "features": [f"{test_dir}/feature_0_train.tfrecord"],
    ...
    def test_extract_single_feature(self):
        ...
        dataset = tf.data.Dataset.from_tensor_slices(
            (tf.constant([[1.0, 2.0], [3.0, 4.0]]), tf.constant([0, 1]))
        )
    ...
    def test_save_feature_shapes_multi_input(self):
        ...
        dataset = tf.data.Dataset.from_tensor_slices(
            (tf.random.uniform((10, 4)), tf.random.uniform((10, 6)))
        )
        dataset = dataset.batch(1)
    ...
    def test_lime_predict(self):
        ...
        result = self.eval._lime_predict(
            X, mock_model, lime_input_shapes, lime_cum_indices
        )
    ...
    def test_extract_target_info_sparse(self, mock_clean_bytes):
        ...
        sample_num = 0
        mock_clean_bytes.side_effect = (
            lambda x: x.decode("utf-8") if isinstance(x, bytes) else x
        )

        data_dict = {
            "target": tf.sparse.from_dense(tf.constant([[0], [1]], dtype=tf.int64)),
            "extra1": tf.sparse.from_dense(
                tf.constant([[b"val1"], [b"val2"]], dtype=tf.string)
            ),
        }

        dataset = tf.data.Dataset.from_tensor_slices(data_dict).batch(1)
        expected_df = pd.DataFrame({"target": [0, 1], "extra1": ["val1", "val2"]})

        result = self.eval._extract_target_info(
            dataset, target_column_names, sample_num
        )
        pd.testing.assert_series_equal(
            result, expected_df.iloc[sample_num], check_dtype=False
        )
    ...
    def test_complete_lime_obs_info(self, mock_connect):
        ...
        target_info = pd.Series({"date": "2025-03-14", "pair": "EURUSD"})
        ...
        with patch("pandas.read_sql_query", return_value=expected_df) as mock_read_sql:
            ...
            mock_read_sql.assert_called_once_with(
                expected_query, mock_conn, params=expected_params
            )
    ...
    def test_fit_cal_fn(self):
        ...
        df = pd.DataFrame(
            {"wl": [0, 1] * 500, "confidence": np.linspace(0.5, 1.0, num=1000)}
        )
        self.eval.cal_config["y_conf"] = "confidence"
        expected_x_list = [
            round(df.iloc[split]["confidence"].mean(), 4)
            for split in np.array_split(df.index, 100)
        ]
        expected_y_list = [
            round(df.iloc[split]["wl"].mean(), 4)
            for split in np.array_split(df.index, 100)
        ]

...
class LoadTrainingData:
    ...
    def chunk_keys(self, mode, source=None, key=None, db_path=None, query=None):
        ...
        with sqlite3.connect(db_path) as conn:
            primary_key_values = [
                tuple(row) for row in conn.execute(select_primary_keys)
            ]
        ...
        for i in range(0, len(primary_key_values), rows_per_chunk):
            ...
            chunk_keys.append(
                (primary_key_values[i], primary_key_values[end_index - 1])
            )
    ...
    @public_method
    def load_chunk(
        self, mode, source=None, key=None, db_path=None, query=None, chunk_key=None
    ):
        """
        ...
        if order_exists:
            ...
            base_query, order_part = (
                base_query[:order_index].strip(),
                base_query[order_index:].strip(),
            )
        else:

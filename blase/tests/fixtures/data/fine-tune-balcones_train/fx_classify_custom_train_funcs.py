...
def custom_loss(y_true, y_pred, max_loss=5, penalty_weight=0.01):
    """
    ...
    y_true_classes = (
        ...
        else tf.cast(y_true, tf.int32)
    )

    # --- Minority Class Penalty ---
    is_minority_class_pred = tf.reduce_any(
        tf.equal(tf.expand_dims(y_pred_classes, axis=-1), minority_classes), axis=-1
    )
    is_minority_class_true = tf.reduce_any(
        tf.equal(tf.expand_dims(y_true_classes, axis=-1), minority_classes), axis=-1
    )
    false_positive_mask = tf.logical_and(
        is_minority_class_pred, tf.logical_not(is_minority_class_true)
    )
...
class CustomEarlyStopping(tf.keras.callbacks.Callback):
    ...
    def on_aggregated_epoch_end(self, epoch, logs):
        ...
        if current < self.best:
            ...
            self.wait = 0
            print(
                f"[EarlyStopping] Epoch {epoch + 1}: {self.monitor} improved to {current:.4f}."
            )
        else:
            self.wait += 1
            print(
                f"[EarlyStopping] Epoch {epoch + 1}: {self.monitor} did not improve (wait {self.wait}/{self.patience})."
            )
    ...
    def on_aggregated_epoch_end(self, epoch, logs=None):
        ...
        if current < self.best:
            ...
            if self.wait >= self.patience:
                ...
                print(
                    f"[ReduceLROnPlateau] Epoch {epoch + 1}: LR reduced from {old_lr:.6e} to {new_lr:.6e}."
                )
...
class CustomModelCheckpoint(tf.keras.callbacks.Callback):
    ...
    def on_aggregated_epoch_end(self, epoch, logs):
        ...
        if current < self.best:
            ...
            print(
                f"[ModelCheckpoint] Epoch {epoch + 1}: Checkpoint saved to {self.filepath} with {self.monitor} = {current:.4f}."
            )
...
class ConfusionMatrixCallback:
    ...
    def log_conf_matrix(self, epoch, all_preds, all_labels):
        cm = confusion_matrix(
            all_labels, all_preds, labels=np.arange(len(self.class_names))
        )
...
class AggregateCallbacks:
    def __init__(
        ...
        # EarlyStopping
        self.early_stopping = (
            CustomEarlyStopping(monitor=monitor_metric, patience=patience)
        ...
        self.reduce_lr = (
            CustomReduceLROnPlateau(
                monitor=monitor_metric, factor=0.8, patience=5, min_lr=1e-6
            )
            if use_reduce_lr
            else None
        )
        ...
        if use_model_checkpoint:
            ...
            self.model_checkpoint = CustomModelCheckpoint(
                filepath=checkpoint_filepath, monitor=monitor_metric
            )
        else:
        ...
        if use_conf_matrix and log_dir is not None:
            ...
            self.conf_matrix_callback = ConfusionMatrixCallback(
                val_log_dir, class_names
            )
    ...
    def on_aggregated_epoch_end(self, epoch, logs, model):
        ...
        if self.csv_log_path is not None:
            ...
            print(
                f"[CSVLogger] Epoch {epoch + 1}: Metrics appended to {self.csv_log_path}."
            )
...
def create_model(
    ...
    x_hourly = LSTM(64, return_sequences=True, kernel_regularizer=l2(l2_strength))(
        hourly_input
    )
    ...
    x_tech = Dense(32, activation=activation, kernel_regularizer=l2(l2_strength))(
        tech_input
    )

    # Engineered Layers
    eng_input = Input(shape=(n_eng_features,))
    x_eng = Dense(32, activation=activation, kernel_regularizer=l2(l2_strength))(
        eng_input
    )

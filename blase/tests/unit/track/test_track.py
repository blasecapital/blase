...
def test_step_context_completes(tracker: Track):
    ...
    with tracker.step("blase.Extract.read_csv", params={"path": "x.csv"}):
        pass
    rows = _q(db, "SELECT status, ts_end, function_fqn FROM steps ORDER BY rowid DESC LIMIT 1")
...
def test_stream_snapshots_code_and_env(tracker: Track):
    db = _db_path(tracker)
    ss = tracker.stream("blase.Transform.apply_function", params={"fn":"_dummy"}, code_fn=_dummy_fn)
    # first emit not last
...
def test_schema_bootstrap_fresh_dir(tmp_path: Path):
    ...
    for t in {"steps", "data", "step_inputs", "step_outputs", 
              "datasets", "dataset_members", "materializations"
...
def test_step_lifecycle_and_signature(tracker: Track):
    ...
    with tracker.step("blase.Extract.read_csv", params={"path": "x.csv"}):
        pass
    row = _q(db, "SELECT status, ts_start, ts_end, sig_hash, attempt_id, step_hash FROM steps ORDER BY rowid DESC LIMIT 1")[0]
...
def test_add_input_and_output_idempotency(tracker: Track):
    ...
    inputs = _q(db, "SELECT data_hash, role FROM step_inputs WHERE step_hash=?", (s.step_hash,))
    outputs = _q(db, "SELECT data_hash, name FROM step_outputs WHERE step_hash=?", (s.step_hash,))
    assert len([r for r in inputs if r["data_hash"]=="h-in" and r["role"]=="source"]) == 1
    assert len([r for r in outputs if r["data_hash"]=="h-out" and r["name"]=="foo"]) == 1
...
def test_register_data_bytes_stores_in_cas_and_data_table(tracker: Track):
    ...
    d = _q(db, "SELECT byte_len, source_path, kind FROM data WHERE data_hash=?", (h,))[0]
...
def test_register_data_path_index_and_copy_policies(tracker: Track, tmp_path: Path, monkeypatch):
...
def test_snapshot_callable_and_env_record_inputs(tracker: Track):
    ...
    roles = {r["role"] for r in _q(db, "SELECT role FROM step_inputs WHERE step_hash=?", (s.step_hash,))}
...
def test_add_upstream_from_meta_and_remove(tracker: Track):
    ...
    with tracker.step("upstream", params={}) as s:
        s.add_upstream_from_meta({"upstream": [{"id": "h1", "role": "seed"}, {"id": "h2"}]})
        s.remove_inputs_by_role("seed")

    rows = _q(db, "SELECT data_hash, role FROM step_inputs WHERE step_hash=?", (s.step_hash,))
...
def test_add_dataset_member_creates_and_appends(tracker: Track):
    ...
    assert ds
    members = _q(db, "SELECT data_hash, ordinal FROM dataset_members WHERE dataset_id='ds1' ORDER BY ordinal")

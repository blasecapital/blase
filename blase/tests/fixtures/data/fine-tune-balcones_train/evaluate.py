...
class Eval:
    ...
    def _initial_bias(self, model_args, weight_dict_path):
        ...
        if not os.path.exists(weight_dict_path):
            raise FileNotFoundError(
                f"Weight dictionary not found at: {weight_dict_path}"
            )
        ...
        initial_bias = np.log(
            [
                class_probs[i] / (1 - class_probs[i] + epsilon)
                for i in sorted(class_probs)
            ]
        )
    ...
    def _model_config(self, config):
        ...
        """
        config_path = os.path.join(config["dir"], config["model_config_src"])
        model_config = self._import_function(config_path, config["config_dict_name"])
        return model_config
    ...
    def _group_pred_files(self, data_dir, feature_category, mode):
        ...
        tfrecord_files = sorted(
            [f for f in os.listdir(data_dir) if f.endswith(".tfrecord")]
        )
        ...
        if mode in ["train", "val", "test"]:
            feature_files = [
                ...
                if f.startswith(feature_category) and mode in f
            ]
            ...
            raise ValueError(
                f"Invalid mode: {mode}. Must be 'full', 'train', 'val', or 'test'."
            )
    ...
    def _load_feature_metadata(self, feature_category, data_dir):
        ...
        # If exact match is missing, try to find a closely matching file
            ...
            matching_files = [
                f for f in json_files if feature_category in os.path.basename(f)
            ]
            ...
            if len(matching_files) == 1:
                ...
                raise FileNotFoundError(
                    f"Multiple feature description JSON files found for {feature_category}: {matching_files}. "
                    f"Please specify the correct one."
                )
            else:
                raise FileNotFoundError(
                    f"Feature description JSON not found for {feature_category}. "
    ...
    def _load_target_metadata(self, data_dir):
        ...
        for key, value in metadata.items():
            if value == "int64":
                metadata[key] = tf.io.FixedLenFeature(
                    [], tf.int64
                )  # Ensure correct int64 format
            elif value == "float32":
                metadata[key] = tf.io.FixedLenFeature(
                    [], tf.float32
                )  # Ensure correct float32 format
            elif value == "string":
                metadata[key] = tf.io.VarLenFeature(
                    tf.string
                )  # Use VarLenFeature for string values
            elif value == "object":
                metadata[key] = tf.io.VarLenFeature(
                    tf.string
                )  # Comvert objects to strings
            else:
                raise ValueError(
                    f"Unsupported data type '{value}' for key '{key}' in target metadata."
                )

        return metadata

    def _parse_tfrecord_fn(
        self, example_proto, feature_metadata, category, feature_categories
    ):
        """
        ...
        for key, dtype in feature_metadata.items():
            ...
            if dtype in ["int64", "int"]:
                ...
                converted_features[key] = tf.strings.to_number(
                    value, out_type=tf.float32
                )
    ...
    def _load_tfrecord_dataset(
        self, feature_files, target_files, feature_categories, data_dir, batch_size
    ):
        """
        ...
        """
        feature_metadata = {
            category: self._load_feature_metadata(category, data_dir)
        ...
        feature_datasets = {
            category: tf.data.TFRecordDataset(files).map(
                lambda x: self._parse_tfrecord_fn(
                    x, feature_metadata[category], category, feature_categories
                ),
        ...
        def parse_target_fn(x):
            ...
            return parsed, parsed[
                "target"
            ]  # Return full parsed targets & just `target` for training
        ...
        feature_tuple_dataset = feature_tuple_dataset.batch(batch_size).prefetch(
            tf.data.AUTOTUNE
        )
        full_target_dataset = full_target_dataset.batch(batch_size).prefetch(
            tf.data.AUTOTUNE
        )
    ...
    def _store_predictions(
        self, predictions, full_target_dataset, target_column_names, dataset_type
    ):
        """
        ...
        epoch_number = (
            self.backtest_config["model_weights"].split("_")[0].replace("epoch", "")
        )
        ...
        try:
            ...
            if not set(primary_keys).issubset(set(target_column_names)):
                raise ValueError(
                    f"Primary keys {primary_keys} are missing from target_column_names."
                )
            ...
            for full_targets in full_target_dataset:  # Looping through target values
                target_dict = {
                    col: (
                        tf.sparse.to_dense(full_targets[col])
                        .numpy()
                        .tolist()  # Convert SparseTensor to dense
                        if isinstance(
                            full_targets[col], tf.sparse.SparseTensor
            ...
            extra_column_definitions = ", ".join(
                [f"{col} REAL" for col in extra_columns]
            )
            ...
            data_to_insert = [
                ...
                + tuple(
                    str(x[0].decode("utf-8"))
                    if isinstance(x, list) and x and isinstance(x[0], bytes)
                    # Convert [b'value'] → 'value'
                    else str(x)
                    ...
                    else str(x)  # Convert all remaining values to strings
    ...
    def _load_model(self, config):
        ...
        create_model = self._import_function(model_modules_path, model_function)
    ...
    def _load_weights(self, model, config):
        ...
        architecture.
        """
        weights_path = os.path.join(config["dir"], config["model_weights"])
    ...
    def _backtest_predict(self, model, mode, config):
        ...
        # target sets
        predict_groups = {
            category: self._group_pred_files(data_dir, category, mode)
            for category in feature_categories
        }
        ...
        for num in sorted(predict_groups[next(iter(feature_categories))].keys()):
            feature_files = {
                category: [
                    f
                    for f in predict_groups[category].get(num, {}).get("features", [])
                    if category
                    in os.path.basename(f)  # Ensures files are correctly matched
                ]
                for category in feature_categories
            }
            target_files = (
                predict_groups[next(iter(feature_categories))]
                .get(num, {})
                .get("targets", [])
            )
            ...
            if feature_files:
                ...
                if sample_file:
                    dataset_type = (
                        os.path.basename(sample_file).split("_")[-1].split(".")[0]
    ...
    def _group_lime_files(self, data_dir):
        ...
        feature_files = [
            f
            for f in tfrecord_files
            if ("features" in f or "feature" in f) and file_num in f
        ]
    ...
    def _save_feature_shapes(self, dataset):
        ...
        for spec in input_specs:
            if isinstance(
                spec, tuple
            ):  # If the spec itself is a tuple (multi-input case)
    ...
    def _lime_predict(self, X, model, lime_input_shapes, lime_cum_indices):
        ...
        for i in range(len(lime_input_shapes)):
            ...
            if input_i_flat.size == 0:
                print(
                    f"Warning: Empty input detected for index {i} (start={start}, end={end}) - Skipping"
                )
    ...
    def _list_cat_cols(self, feature_names):
        ...
        categorical_features = [
            col for col in feature_names if re.search(cat_feature_pattern, col)
        ]
        continuous_features = [
            col for col in feature_names if col not in categorical_features
        ]
    ...
    def _extract_target_info(
        self, full_target_dataset, target_column_names, sample_num
    ):
        """
            ...
            current_target_dict = {
                col: (
                    [
                        ...
                        for val in tf.sparse.to_dense(full_targets[col])
                        .numpy()
                        .tolist()
                    ]
                    if isinstance(full_targets[col], tf.sparse.SparseTensor)
                        ...
                        val[0] if isinstance(val, list) and len(val) == 1 else val
                        for val in full_targets[col].numpy().tolist()
                    ]
                )
                for col in target_column_names
            }
    ...
    def _complete_lime_obs_info(self, target_info):
        ...
        try:
            ...
            id_cols = self.explain_config[
                "id_cols"
            ]  # List of column names like ["date", "pair", "target"]

            # Ensure all id_cols exist in target_info
            if not all(col in target_info.index for col in id_cols):
                raise ValueError(
                    f"Missing required ID columns in target_info: {id_cols}"
                )
    ...
    def _load_lime_data(self, config):
        ...
        # target sets
        predict_groups = {
            category: self._group_lime_files(data_dir)
            for category in feature_categories
        }
    ...
    def _run_lime(
        self, dataset, feature_names, full_target_dataset, target_column_names, model
    ):
        """
        ...
        target_info = self._extract_target_info(
            full_target_dataset, target_column_names, sample_num
        )
        ...
        predict_fn = functools.partial(
            self._lime_predict,
            model=model,
            lime_input_shapes=lime_input_shapes,
            lime_cum_indices=lime_cum_indices,
        )

        explanation = explainer.explain_instance(X_sample[sample_num], predict_fn)
    ...
    def explain(self):
        ...
        dataset, feature_names, full_target_dataset, target_column_names = (
            self._load_lime_data(config)
        )
        self._run_lime(
            dataset, feature_names, full_target_dataset, target_column_names, model
        )
    ...
    def _f1_score(self, y_true, y_pred, average="weighted", zero_division=0):
        ...
        """
        return round(
            f1_score(y_true, y_pred, average=average, zero_division=zero_division), 4
        )
    ...
    def _compute_metrics(self, y_true, y_pred, y_conf):
        ...
        if "precision" in metrics:
            results["precision"] = self._precision_score(
                y_true, y_pred, average="weighted", zero_division=0
            )
        if "recall" in metrics:
            results["recall"] = self._recall_score(
                y_true, y_pred, average="weighted", zero_division=0
            )
        if "f1_score" in metrics:
            results["f1_score"] = self._f1_score(
                y_true, y_pred, average="weighted", zero_division=0
            )
        if "log_loss" in metrics:
            results["log_loss"] = (
                self._log_loss(y_true, y_pred, y_conf) if len(set(y_true)) > 1 else None
            )
        if "roc_auc" in metrics:
            results["roc_auc"] = (
                self._roc_auc_score(y_true, y_pred, y_conf, multi_class="ovo")
                if len(set(y_true)) > 1
                else None
            )
    ...
    def _display_results(self, db_path, query, mode="convert"):
        ...
        overall_metrics = self._compute_metrics(
            df[y_true_col], df[y_pred_col], df[y_conf_col]
        )
        ...
        for split in metric_cats["split_col"]:
            ...
            split_metrics = self._compute_metrics(
                df_split[y_true_col], df_split[y_pred_col], df_split[y_conf_col]
            )
            ...
            for pair in metric_cats["asset_col"]:
                ...
                pair_metrics = self._compute_metrics(
                    df_pair[y_true_col], df_pair[y_pred_col], df_pair[y_conf_col]
                )
            ...
            for pair in metric_cats["asset_col"]:
                ...
                for target in metric_cats["target_col"]:
                    ...
                    target_metrics = self._compute_metrics(
                        df_target[y_true_col],
                        df_target[y_pred_col],
                        df_target[y_conf_col],
                    )
            ...
            if target_data:
                target_df = pd.DataFrame(target_data).set_index(
    ...
    @public_method
    def report_metrics(self, display_mode="original"):
        """
    ...
    @public_method
    def report_calibration(self, mode="original"):
        """
    ...
    def _find_candidates(self, df, accuracy_threshold, volume):
        ...
        filtered_candidates = [
            ...
            for asset, data in candidate_dict.items()
            if data
    ...
    @public_method
    def report_candidates(self, mode="convert", accuracy_threshold=0.35, volume=300):
        """
        Load the predictions from the database specified in the config, calculate

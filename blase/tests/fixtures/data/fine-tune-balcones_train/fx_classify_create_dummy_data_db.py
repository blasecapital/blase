...
def populate_dummy_table(db_path, n_samples):
    ...
    data = {"date": [], "pair": [], "open": [], "high": [], "low": [], "close": []}
    ...
    for i, pair in enumerate(pair_dict.keys()):
        ...
        for j in range(n_samples):
            ...
            if j == 0:
                ...
                open_price = round(
                    data["close"][(i * n_samples) + j - 1]
                    * (1 + np.random.uniform(low=-1.0) * 0.000001),
                    5,
                )
            ...
            if np.random.rand() < trend_scaler:
                if mid_rand > 0:
                    close_price = high_price - (
                        (high_price - close_price) * np.random.rand()
                    )
                else:
                    close_price = (
                        (close_price - low_price) * np.random.rand()
                    ) + low_price

...
def feature_engineering(df):
    ...
    if df["date"].isna().any():
        raise ValueError(
            "Some dates could not be converted to datetime. Check 'date' column."
        )
...
def target_engineering(df):
    ...
    df["target"] = np.where(eod_condition | condition_hours_passed, 0, df["target"])
...
def df_features(df):
    ...
    return [
        ...
        if col
        not in ["date", "pair", "target", "hours_passed", "buy_sl_time", "sell_sl_time"]
    ]

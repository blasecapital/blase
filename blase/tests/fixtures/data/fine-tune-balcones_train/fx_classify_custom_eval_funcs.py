...
def calculate_running_profit(df, initial_value=100000):
    ...
    for i in range(len(df)):
        ...
        for trade_pair in list(open_trades.keys()):
            new_open_trades = []
            for trade_time, trade_signal, trade_hours_passed, trade_wl in open_trades[
                trade_pair
            ]:
                time_difference = (
                ...
                if time_difference >= trade_hours_passed:
                    ...
                    new_open_trades.append(
                        (trade_time, trade_signal, trade_hours_passed, trade_wl)
                    )
                ...
                if not open_trades[pair] or open_trades[pair][0][1] == predicted_target:
                    if trade_hours_passed is not None:
                        open_trades[pair].append(
                            (current_time, predicted_target, trade_hours_passed, wl)
                        )
                        trade_opened = 1
    ...
    time_elapsed_years = (
        (df["date"].iloc[-1] - df["date"].iloc[0]).total_seconds() / 31536000
    ) - 1  # Time in years adjusted for missing data
    annual_rate_of_return = (
        (final_profit / initial_profit) ** (1 / time_elapsed_years) - 1
    ) * 100

...
class CreateFeatures:
    ...
    def _import_feature_module(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "feature_module", self.module_path
        )
    ...
    def _import_storage_map(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "storage_map_module", self.module_path
        )
        ...
        with sqlite3.connect(db_path) as conn:
            for table, columns in storage_map.items():
                ...
                if not table_exists:
                    ...
                    column_defs = ", ".join(
                        [f"{col} TEXT" for col in primary_key + columns]
                    )
    ...
    def _store_original_columns(self, df):
        ...
        targets.
        """
        original_columns = [col for col in df.columns if col not in self.primary_key]
    ...
    def _process_and_save_dataframe(self, df, original_columns):
        ...
        feature_df = self._drop_base_data_columns(
            processed_df, original_columns
        )  # Remove base columns
    ...
    def _process_and_save_grouped(self, grouped_df, original_columns):
        ...
        for _, group in grouped_df:
            ...
            self.store_features(
                feature_df, self.config["feature_source"]
            )  # Save features
            del processed_df, feature_df  # Free memory
            gc.collect()

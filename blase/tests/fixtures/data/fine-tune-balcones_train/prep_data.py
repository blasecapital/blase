...
class PrepData:
    ...
    def engineer(self, mode="all"):
        ...
        if mode == "feature":
            ...
            raise ValueError(
                f"Invalid mode: {mode}. Choose from 'all', 'feature', or 'target'."
            )
            ...
            for table in table_list:
                ...
                for idx, chunk_key in enumerate(chunk_keys):
                    ...
                    if idx < int(len(chunk_keys) * 0.6):
                        ...
                        data = data.drop(columns=self.primary_key, errors="ignore")
                ...
                scaler_save_path = os.path.join(
                    self.scaler_save_dir, f"{table}_scaler" + ".pkl"
                )
    ...
    def _convert_sqlite_text_to_types(self, df):
        ...
        for col in df.columns:
            sample_values = (
                df[col].dropna().unique()[: min(100, len(df[col]))]
            ...
            is_numeric = any(
                re.match(r"^-?\d+(\.\d+)?$", str(val)) for val in sample_values
            )
    ...
    def _save_feature_description(self, table, data):
        ...
        json_path = os.path.join(
            self.prepped_data_dir, f"{table}_feature_description.json"
        )
    ...
    def _save_tfrecord_chunk(self, data, filename):
        ...
        def serialize_example(row):
            feature_dict = {
                col: self._int64_feature(getattr(row, col))
                if isinstance(getattr(row, col), (np.integer, int))
                else self._float_feature(getattr(row, col))
                if isinstance(getattr(row, col), (np.floating, float))
                else self._bytes_feature(str(getattr(row, col)))
    ...
    def _create_and_save_weights_dict(self, data, target_col, fin):
        ...
        if fin:
            ...
            final_weights = {
                ...
                for cls, count in self.weights_dict.items()
            }
            ...
            print(
                "Final class weights saved to {}: {}".format(
    ...
    def save_batches(self):
        ...
        print(
            f"Using global chunk keys from table '{max_table}' with {len(global_chunk_keys)} chunks for alignment."
        )
        ...
        for key in self.prep_and_save_dict.keys():
            ...
            for table in table_list:
                ...
                for idx, chunk_key in enumerate(global_chunk_keys):
                    print(f"Processing chunk {chunk_key[0][0]} - {chunk_key[1][0]}...")
                    data = self.ltd.load_chunk(
                        mode="manual", db_path=db_path, query=query, chunk_key=chunk_key
                    )

                    # Remove primary keys before scaling
                    if not self.prep_and_save_dict[key][1][table].get(
                        "keep_primary_key"
                    ):
                        data = data.drop(columns=self.primary_key, errors="ignore")
                    ...
                    if scale:
                        data_columns = [
                            col for col in data.columns if col not in self.primary_key
                        ]  # Features only
                        ...
                        scaled_df = pd.DataFrame(
                            scaled_data, columns=data_columns, index=data_index
                        )
                    ...
                    if idx < n_train:
                        ...
                        if self.prep_and_save_dict[key][1][table].get("weights_dict"):
                            target_col = self.prep_and_save_dict[key][1][table][
                                "weights_dict"
                            ]
                            fin = idx == n_train - 1
                    ...
                    filename = os.path.join(
                        self.prepped_data_dir, f"{table}_{idx}_{split_type}.tfrecord"
                    )
    ...
    def validate_record(self, max_features=10):
        ...
        for file in file_list:
            ...
            for raw_record in raw_dataset.take(1):
                ...
                print(
                    f"{file} - Displaying {len(selected_features)} / {len(feature_dict)} features:"
                )

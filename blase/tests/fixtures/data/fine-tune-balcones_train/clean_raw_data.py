...
class CleanRawData:
    ...
    def _calculate_percentiles(self, bin_edges, bin_frequencies, total_count):
        ...
        cumulative_frequency = np.cumsum(
            frequencies
        )  # Cumulative frequency for interpolation
            ...
            if bin_idx == 0:
                ...
                if bin_frequency > 0:
                    ...
                    interpolated_value = bin_start + (
                        (target_count - prev_cumulative)
                        / (curr_cumulative - prev_cumulative)
                    ) * (bin_end - bin_start)
    ...
    def _finalize_describe_report(self, key, progress_log):
        ...
        for feature in progress_log["running_sum"]:
            ...
            variance = progress_log["running_square_sum"][feature] / count - mean**2
    ...
    def _initialize_bins_from_sql(self, col_list, key):
        ...
        def process_feature(feature):
            ...
            try:
                ...
                if result and result[0] is not None and result[1] is not None:
                    ...
                    raise ValueError(
                        f"No valid data found for feature '{feature}' in table '{table_name}' with filter '{where_clause}'."
                    )
        ...
        with ThreadPoolExecutor() as executor:
            ...
            with tqdm(
                total=len(futures), desc="Processing Features", unit="feature"
            ) as pbar:
    ...
    def _plot_descriptive_stats(self, progress_log):
        ...
        for feature, stats in progress_log["final_stats"].items():
            ...
            valid_bins = [
                ...
                if lower_bound <= edge <= upper_bound
            ]
            ...
            plt.title(
                f"Feature: {feature} - Bin Distribution with Overflow & Underflow"
            )
            plt.xlabel("Bins")
    ...
    def _plot_rates(self, data, plot_skip):
        ...
        for col in data.columns:
            ...
            if match:
                ...
                feature_groups[num][ohlc_type] = (
                    col  # Store column names under OHLC category
                )

        # Ensure each group contains all four OHLC components
        valid_groups = {
            num: cols for num, cols in feature_groups.items() if all(cols.values())
        }
        ...
        for index in range(0, len(data), plot_skip):
            ...
            ohlc_data = pd.DataFrame(
                {
                    ...
                    "Close": [
                        row[valid_groups[num]["close"]] for num in sorted_numbers
                    ],
                }
            )
            ...
            date_range = pd.date_range(
                start=dummy_start_date, periods=len(sorted_numbers), freq="H"
            )  # Hourly intervals
    ...
    def _describe_features(
        self, key, chunk_key, data, progress_log, bin_edges, col_list, finish
    ):
        """
        ...
        for feature in col_list:
            if feature not in progress_log["running_sum"]:
                ...
                progress_log["bin_frequencies"][feature] = {
                    i: 0 for i in range(len(bin_edges[feature]) - 1)
                }
                progress_log["bin_frequencies"][feature].update(
            ...
            if feature in chunk_stats.index:
                ...
                progress_log["running_square_sum"][feature] += (
                    chunk_stats.loc[feature, "std"] ** 2
                    + chunk_stats.loc[feature, "mean"] ** 2
                ) * chunk_stats.loc[feature, "count"]
                progress_log["running_count"][feature] += chunk_stats.loc[
                    feature, "count"
                ]
                progress_log["running_min"][feature] = min(
                    progress_log["running_min"][feature],
                    chunk_stats.loc[feature, "min"],
                )
                progress_log["running_max"][feature] = max(
                    progress_log["running_max"][feature],
                    chunk_stats.loc[feature, "max"],
                )
    ...
    def _display_cat_target_stats(self, progress):
        ...
        target_counts_df = pd.DataFrame.from_dict(
            progress["cat_counts"]["target"], orient="index"
        ).T
        ...
        with pd.option_context(
            ...
            print(
                pd.concat([target_counts_df, percent_row])
    ...
    def _describe_targets(self, key, chunk_key, data, col_list, finish, target_type):
        """
        ...
        if target_type == "cat":
            ...
            for col in col_list:
                ...
                for category, count in counts.items():
                    progress["cat_counts"][col][category] = (
                        progress["cat_counts"][col].get(category, 0) + count
                    )
            ...
            for time_col in ["hours_passed", "buy_sl_time", "sell_sl_time"]:
                if time_col in data.columns:
                    if time_col not in progress["time_bins"]:
                        progress["time_bins"][time_col] = {
                            label: 0 for label in time_labels
                        }
                    ...
                    for label, count in time_counts.items():
                        progress["time_bins"][time_col][label] += (
                            count  # No more KeyError
                        )
            ...
            for col in col_list:
                ...
                progress["quant_stats"]["min"][col] = min(
                    progress["quant_stats"]["min"][col], chunk.min()
                )
                progress["quant_stats"]["max"][col] = max(
                    progress["quant_stats"]["max"][col], chunk.max()
                )
        ...
        if target_type not in {"cat", "quant"}:
            raise ValueError(
                f"Invalid target_type: {target_type}. Use 'cat' or 'quant'"
            )
    ...
    def _collect_cols(self, key):
        ...
        if "*" in columns:
            ...
            try:
                with sqlite3.connect(db_path) as conn:
                    ...
                    ]  # Assuming column names are in the second position
            except Exception as e:
        ...
        dummy_df = pd.DataFrame(
            columns=columns
        )  # Create a dummy DataFrame with the column names
    ...
    def _remove_primary_key(self, data):
        ...
        if missing_keys:
            raise ValueError(
                f"Primary key columns not found in dataframe: {missing_keys}"
            )
        ...
        if any([describe_features, describe_targets, plot_features]):
            ...
            for key in data_keys:
                ...
                chunk_keys = self.ltd.chunk_keys(
                    mode="config", source=source, key=key
                ...
                for idx, chunk_key in enumerate(chunk_keys):
                    ...
                    if idx == 0:
                        if not (
                            plot_features
                            and plot_mode == "rate"
                            and not any([describe_features, describe_targets])
                        ):
                            col_list = self._collect_cols(key)
                            if not describe_targets:
                                bin_edges = self._initialize_bins_from_sql(
                                    col_list, key
                                )

                    data = self.ltd.load_chunk(
                        mode="config", source=source, key=key, chunk_key=chunk_key
                    )
                    ...
                    if describe_targets:
                        self._describe_targets(
                            key, chunk_key, data, col_list, finish, target_type
                        )
                    if plot_features:
                        if plot_mode == "rate":
    ...
    def clean(self, data_keys=[]):
        ...
        def clean_steps(data_keys):
            for key in data_keys:
                ...
                chunk_keys = self.ltd.chunk_keys(mode="config", source=source, key=key)
    ...
    def _remove_bad_keys(self, data, bad_key_set):
        ...
        for key in primary_keys:
            if key not in data.columns:
                raise KeyError(
                    f"Primary key '{key}' not found in data columns: {data.columns}"
                )

        # Filter out rows that have a primary key match in bad_key_set
        mask = data.apply(
            lambda row: (row["pair"], row["date"]) not in bad_key_set, axis=1
        )
            ...
            for db_path in db_paths:
                ...
                try:
                    ...
                    for table in tables:
                        ...
                        """
                        chunk_keys = self.ltd.chunk_keys(
                            mode="manual", db_path=db_path, query=query
                        )
                        for idx, chunk_key in enumerate(chunk_keys):
                            print(
                                f"Processing chunk {chunk_key[0][0]} - {chunk_key[1][0]}..."
                            )
                            data = self.ltd.load_chunk(
                                mode="manual",
                                db_path=db_path,

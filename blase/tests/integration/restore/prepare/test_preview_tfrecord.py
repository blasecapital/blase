...
def workspace(tmp_path, monkeypatch):
    project_root = tmp_path / "proj_preview_tfrecord_restore"
...
def test_preview_tfrecord_step_and_data_restore(tmp_path, monkeypatch):
    ...
    tf_paths = [
        Path(a.path) for a in (res.artifacts or []) if a.path.endswith(".tfrecord")
    ]
    assert tf_paths, "no TFRecord artifacts written"
    tf_path = tf_paths[0]

    # Preview TFRecord (tracked) – write thumbs to a dir
    prev_out = project_root / "data" / "working" / "tfrecord_preview"
    if prev_out.exists():
        for p in prev_out.glob("*"):
            p.unlink()
    prev_out.mkdir(parents=True, exist_ok=True)

    _ = prep.preview_tfrecord(
        path=tf_path,
        n=6,
        compression="GZIP",
        decode_images=True,
    ...
    # Confirm thumbs written
    thumbs = list(prev_out.glob("*"))
    assert thumbs, "preview_tfrecord produced no files"
    assert all(p.stat().st_size > 0 for p in thumbs)

    # Locate the recorded preview step + outputs
    step_hash = latest_step(project_root, "blase.Prepare.preview_tfrecord")
    outs = outputs_for_step(project_root, step_hash)
    assert outs, "preview_tfrecord step recorded no outputs"
    # Prefer an image-looking name, else first
    img_out = next(
        (
            o
            for o in outs
            if (o["name"] or "").lower().endswith((".png", ".jpg", ".jpeg"))
        ),
        outs[0],
    )
    img_hash = img_out["data_hash"]

    # ---- STEP RESTORE: replay preview into a fresh directory
    replay_dir = project_root / "data" / "working" / "tfrecord_preview_replay_step"
    ...
    rep_files = list(replay_dir.glob("*"))
    assert rep_files, "step replay produced no preview files"
    assert all(p.stat().st_size > 0 for p in rep_files)

    # ---- DATA RESTORE: materialize one specific preview image by data hash
    data_target = project_root / "data" / "working" / f"{img_hash}.png"
    if data_target.exists():
        data_target.unlink()
    rc2 = cli_restore_data_to(project_root, img_hash, data_target)

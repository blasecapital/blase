...
def run_path(tmp_path):
    ...
    (runs / "active_run.blase").write_text(json.dumps({"run_id": rid, "status": "active"}))
    # minimal schema
    DDL = [
...
def _args(**kw):
    ...
    base = dict(
        run=None, mode="replay", data="DHASH", step=None,
        to=None, on_conflict="overwrite", keep_intermediates=False,
        limit_batches=None, backend=None,
    )
    base.update(kw)
...
def isolate_cmd_run(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(rc, "_resolve_run_path", lambda run: tmp_path)
    rd = tmp_path / "restore"; rd.mkdir(parents=True, exist_ok=True)
...
def test_exec_plan_read_images_builds_realized_and_calls_restore(monkeypatch, run_path, tmp_path):
    sh = "step_read_images"
    # Fake planner: one-step plan
    monkeypatch.setattr(rc, "planner", SimpleNamespace(plan_for_step=lambda rp, tip: [sh]))
    # Fake store
    params = {"directory": "/data/images", "pattern": "**/*.jpg", "mode": "auto"}
    st_row = {"function_fqn": "blase.Extract.read_images", "params": params}
    ins = [{"data_hash": "CODE123", "role": "code"}, {"data_hash": "ENV999", "role": "env"}]
    outs = [
    ...
    store_fake = SimpleNamespace(
        ...
        pick_code_hash=lambda ins_list: next(i["data_hash"] for i in ins_list if i["role"] == "code"),
    )
    ...
    def _restore(**kw):
        ...
        return _gen()
    monkeypatch.setattr(rc, "bindings", SimpleNamespace(run_read_images_restore=_restore))
...
def test_exec_plan_read_images_missing_directory_omits_source(monkeypatch, run_path):
    sh = "step_read_images"
    monkeypatch.setattr(rc, "planner", SimpleNamespace(plan_for_step=lambda rp, tip: [sh]))
    st_row = {"function_fqn": "blase.Extract.read_images", "params": {"pattern": "*.jpg"}}
...
def test_apply_function_with_source_anchor(monkeypatch, run_path):
    ...
    monkeypatch.setattr(rc, "cas", SimpleNamespace(path_for=lambda rp, kind, h: run_path / "cas" / h))
    monkeypatch.setattr(rc, "code", SimpleNamespace(load_callable_from_blob=lambda p: _dummy_fn))

    # resolve source path
    monkeypatch.setattr(rc, "_resolve_source_no_copy", lambda rp, h, seen_steps, created_paths: ("/data/src.csv", False))
    ...
    monkeypatch.setattr(
        ...
        "bindings",
        SimpleNamespace(
            run_apply_function_restore=lambda **kw: called.setdefault("kw", kw) or (x for x in ()),
        ),
    )
...
def test_apply_function_with_manifest_anchor(monkeypatch, run_path):
    ...
    store_fake = SimpleNamespace(
        ...
        read_images_params_for_manifest=lambda rp, mh, ts_before: {"directory": "/imgs", "pattern": "**/*.jpg"},
    )
    monkeypatch.setattr(rc, "store", store_fake)
...
def test_apply_function_manifest_anchor_missing_params_raises(monkeypatch, run_path):
    ...
    store_fake = SimpleNamespace(
        ...
        load_step_outputs=lambda rp, s: [],
        pick_code_hash=lambda ins_list: "C",
        read_images_params_for_manifest=lambda rp, mh, ts_before: None,  # critical: missing
    )
    monkeypatch.setattr(rc, "store", store_fake)
    monkeypatch.setattr(rc, "cas", SimpleNamespace(path_for=lambda *a, **k: Path("/x")))
    monkeypatch.setattr(rc, "code", SimpleNamespace(load_callable_from_blob=lambda p: _dummy_fn))
...
def test_parquet_with_recorded_outs_and_override(monkeypatch, tmp_path):
    ...
    st_row = {"function_fqn": "blase.Load.save_images_to_parquet", "params": {"shard_prefix": "b"}}
    store_fake = SimpleNamespace(
        load_step=lambda rp, s: st_row,
        load_step_inputs=lambda rp, s: [],
        load_step_outputs=lambda rp, s: outs,
    ...
    made_gen = object()
    called_upstream = {}
    monkeypatch.setattr(rc, "_upstream_gen_for_sink", lambda rp, s: called_upstream.setdefault("gen", made_gen))
    ...
    monkeypatch.setattr(
        rc,
        "bindings",
        SimpleNamespace(RESTORE_HANDLERS={"blase.Load.save_images_to_parquet": fake_handler}),
    )
    ...
    produced = rc._exec_plan_for_step(
        run_path=tmp_path, tip_step_hash=sh, to_path=override_dir, backend_override=None,
        seen_steps=None, created_paths=created_paths, ephemeral_only=False,
    )
    ...
    assert created_paths == [Path(override_dir) / "b_1.parquet", Path(override_dir) / "b_2.parquet"]


def test_parquet_without_recorded_outs_uses_default_dir_and_hashes(monkeypatch, tmp_path):
    sh = "step_parquet"
    monkeypatch.setattr(rc, "planner", _mk_planner_single(sh))

    st_row = {"function_fqn": "blase.Load.save_images_to_parquet", "params": {"shard_prefix": "c"}}
    store_fake = SimpleNamespace(
        load_step=lambda rp, s: st_row,
        load_step_inputs=lambda rp, s: [],
    ...
    class FakeHash:
        def hash_file(self, p): return f"HASH::{Path(p).name}"
    monkeypatch.setattr(rc, "Hash", FakeHash)

    created_paths = []
    produced = rc._exec_plan_for_step(
        run_path=tmp_path, tip_step_hash=sh, to_path=None, backend_override=None,
        seen_steps=None, created_paths=created_paths, ephemeral_only=False,
    )
...
def test_read_csv_branch(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(rc, "store", SimpleNamespace(
        load_step=lambda rp, s: st_row,
        load_step_inputs=lambda rp, s: ins,
    ))
    ...
    monkeypatch.setattr(rc, "bindings", SimpleNamespace(run_read_csv_restore=run_read_csv_restore))
...
def test_read_images_branch(monkeypatch, tmp_path):
    sh = "S2"
    st_row = {"function_fqn": "pkg.Extract.read_images", "params": {"directory": "/imgs"}}
    ins = [
    ...
    monkeypatch.setattr(rc, "bindings", SimpleNamespace(run_read_images_restore=run_read_images_restore))
...
def test_transform_branch_uses_plan_upstream(monkeypatch, tmp_path):
    ...
    nodes = [{"step_hash": up_sh, "function_fqn": "pkg.Extract.read_csv"},
             {"step_hash": t_sh,  "function_fqn": "pkg.Transform.apply_function"}]
    monkeypatch.setattr(rc, "planner", SimpleNamespace(
        plan_for_step=lambda rp, s: [n["step_hash"] for n in nodes],
        _db=lambda rp: None, _step_row=lambda *_: {"ts_start": "X"},
    ))
    ...
    # code loader and cas
    monkeypatch.setattr(rc, "cas", SimpleNamespace(path_for=lambda rp, kind, h: tmp_path / "code" / h))
    monkeypatch.setattr(rc, "code", SimpleNamespace(load_callable_from_blob=lambda p: lambda x: x))

    # bindings
    up_gen = object()
    def run_read_csv_restore(**kw): return up_gen
    ...
    monkeypatch.setattr(rc, "bindings", SimpleNamespace(
...
def test_transform_branch_lineage_manifest(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(rc, "planner", SimpleNamespace(
        ...
        _db=lambda rp: None, _step_row=lambda *_: {"ts_start": "Y"},
    ))
    monkeypatch.setattr(rc, "_normalize_plan_nodes", lambda rp, raw: [{"step_hash": t_sh, "function_fqn": "pkg.Transform.apply_function"}])
    ...
    ex_st = {"function_fqn": "pkg.Extract.read_images", "params": {"directory": "/imgs"}}
    ins_tr = [{"role": "code", "data_hash": "CODE2"}, {"role": "manifest", "data_hash": "MANI2"}, {"role": "batch", "data_hash": "B"}]
    ...
    # cas/code
    monkeypatch.setattr(rc, "cas", SimpleNamespace(path_for=lambda rp, kind, h: tmp_path / "code" / h))
    monkeypatch.setattr(rc, "code", SimpleNamespace(load_callable_from_blob=lambda p: lambda y: y))

    # bindings for images + apply
    def run_read_images_restore(**kw): return "IMG_GEN"
    captured = {}
    def run_apply_function_restore(**kw):
        captured.update(kw)
        return "AF2"
...
def test_transform_branch_missing_anchor_raises(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(rc, "planner", SimpleNamespace(
        plan_for_step=lambda rp, s: [t_sh],
        _db=lambda rp: None, _step_row=lambda *_: {"ts_start": "Z"},
    ))
    monkeypatch.setattr(rc, "_normalize_plan_nodes", lambda rp, raw: [{"step_hash": t_sh, "function_fqn": "pkg.Transform.apply_function"}])
    monkeypatch.setattr(rc, "_is_stream_fqn", lambda fqn: False)
...
def test_replay_fallback_to_to_path_when_not_in_mapping(monkeypatch, setup_env):
    ...
    monkeypatch.setattr(rc, "_exec_plan_for_step",
                        lambda *a, **k: {"OTHER": Path("irrelevant.csv")})
...
def test_replay_fallback_to_restore_default_name(monkeypatch, setup_env):
    ...
    def _producer(rp, h): return "SINKSTEP"
    def _load_step(rp, sh): return {"params": {"target": default_name}}
...
def test_replay_verify_branch_with_non_sink_stream(monkeypatch, setup_env):
    ...
    def _producer(rp, h): return "STEPX"
    # MUST include params.target so fallback default path can be computed
    default_name = "fallback.csv"
    def _load_step(rp, sh): return {"function_fqn": "blase.Extract.read_csv",
...
def test_replay_sink_calls_handler(monkeypatch, isolate_cmd_run):
    tmp_path, _rd, store_extras = isolate_cmd_run
    st = {"function_fqn": "blase.Load.save_images_to_parquet", "params": {"shard_prefix": "x"}}
    fake_store = SimpleNamespace(load_step=lambda rp, sh: st, **store_extras)
    monkeypatch.setattr(rc, "store", fake_store)
    ...
    monkeypatch.setattr(
        rc,
        "bindings",
        SimpleNamespace(RESTORE_HANDLERS={"blase.Load.save_images_to_parquet": handler}),
    )

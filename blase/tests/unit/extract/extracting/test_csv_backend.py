...
def temp_csv_file(tmp_path_factory):
    ...
    df = pd.DataFrame({"id": range(10), "value": [f"text_{i}" for i in range(10)]})
...
def test_memory_aware_batcher_pandas(temp_csv_file):
    batch_size = memory_aware_batcher(
        file_path=temp_csv_file, backend="pandas", verbose=True
    )
...
def test_memory_aware_batcher_polars(temp_csv_file):
    try:
        batch_size = memory_aware_batcher(
            file_path=temp_csv_file, backend="polars", verbose=True
        )
        assert isinstance(batch_size, int)
        assert batch_size > 0
    except ImportError:
...
def test_memory_aware_batcher_missing_file():
    with pytest.raises(FileNotFoundError):
        memory_aware_batcher(
            file_path="nonexistent.csv", backend="pandas", verbose=True
        )
...
def test_read_batches_pandas_with_use_cols_only(sample_csv_file):
    batches = list(
        read_batches_pandas(
            sample_csv_file, batch_size=2, use_cols=["id", "country"], filter_by=None
        )
    )

    assert len(batches) == 3  # 2 + 2 + 1

    for i, (batch, is_last) in enumerate(batches):
        assert isinstance(batch, pd.DataFrame)
        assert "id" in batch.columns
        assert "country" in batch.columns
        assert "year" not in batch.columns  # ensure it's excluded

    assert batches[0][0].iloc[0]["id"] == 1  # access batch 0, then first row "id"
    assert batches[-1][1] is True  # make sure the final batch is flagged


def test_read_batches_pandas_with_use_cols_subset(sample_csv_file):
    batches = [
        batch
        for batch, _ in read_batches_pandas(
            sample_csv_file, batch_size=3, use_cols=["country"], filter_by=None
        )
    ]
...
def test_read_batches_pandas_no_filter(sample_csv_file):
    batches = [
        batch
        for batch, _ in read_batches_pandas(
            sample_csv_file, batch_size=2, use_cols=None, filter_by=None
        )
    ]
...
def test_read_batches_pandas_multiple_filters(sample_csv_file):
    filters = [{"col": "country", "value": "USA"}, {"col": "year", "value": 2020}]

    batches = [
        batch
        for batch, _ in read_batches_pandas(
            sample_csv_file, batch_size=2, use_cols=None, filter_by=filters
        )
    ]
...
def test_read_batches_polars_basic(sample_csv_file):
    batches = [
        batch
        for batch, _ in read_batches_polars(
            sample_csv_file, batch_size=2, use_cols=None, filter_by=None
        )
    ]

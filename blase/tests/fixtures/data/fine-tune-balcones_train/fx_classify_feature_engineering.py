...
def standardize_open_hourly_rates(df, shift=14):
    ...
    shifted_data = {
        f"open_standard_{hour + 1}": df["open"].shift(hour) for hour in range(shift)
    }
...
def standardize_high_hourly_rates(df, shift=14):
    ...
    # Create all shifted columns at once
    shifted_data = {
        f"high_standard_{hour + 1}": df["high"].shift(hour) for hour in range(shift)
    }
    shifted_data["baseline_close"] = df["close"].shift(shift)
...
def standardize_low_hourly_rates(df, shift=14):
    ...
    # Create all shifted columns at once
    shifted_data = {
        f"low_standard_{hour + 1}": df["low"].shift(hour) for hour in range(shift)
    }
    shifted_data["baseline_close"] = df["close"].shift(shift)
...
def standardize_close_hourly_rates(df, shift=14):
    ...
    # Create all shifted columns at once
    shifted_data = {
        f"close_standard_{hour + 1}": df["close"].shift(hour) for hour in range(shift)
    }
    shifted_data["baseline_close"] = df["close"].shift(shift)
...
def high_vol(df):
    ...
    df["high_vol"] = np.where(df["avg_range"] > 0.002, 1, 0)
    return df["high_vol"]
...
storage_map = {
    ...
    "test_technical_feature_data": ["high_vol", "sma_roc", "close_to_std_dev"],
}

...
def load_step(run_path: Path, step_hash: str) -> Dict[str, Any]:
    ...
    with _conn(db) as c:
        row = c.execute(
            "SELECT function_fqn, params_json, status FROM steps WHERE step_hash=?",
            (step_hash,),
        ).fetchone()
    if not row:
...
def lookup_source_path(run_path: Path, data_hash: str) -> Optional[str]:
    ...
    with _conn(db) as c:
        row = c.execute(
            "SELECT source_path FROM data WHERE data_hash=?", (data_hash,)
        ).fetchone()
    if not row:
        return None
...
def load_materializations(run_path: Path, data_hash: str) -> List[str]:
    ...
    with _conn(db) as c:
        rows = c.execute(
            "SELECT path FROM materializations WHERE data_hash=?", (data_hash,)
        ).fetchall()
...
def drop_materialization(run_path: Path, data_hash: str, path: str) -> None:
    ...
    with _conn(db) as c:
        c.execute(
            "DELETE FROM materializations WHERE data_hash=? AND path=?",
            (data_hash, path),
        )
        c.commit()
    ...
    with _conn(db) as c:
        c.execute(
            """INSERT OR IGNORE INTO materializations(data_hash, path, ts)
                     VALUES (?,?,?)""",
            (data_hash, path, strftime("%Y-%m-%dT%H:%M:%S")),
        )
        c.commit()
...
def producer_step_for_data(run_path: Path, data_hash: str) -> Optional[str]:
    ...
    with _conn(db) as c:
        row = c.execute(
            "SELECT step_hash FROM step_outputs WHERE data_hash=?", (data_hash,)
        ).fetchone()
    return row[0] if row else None


def read_images_params_for_manifest(
